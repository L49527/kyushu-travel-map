        ...
        <div class="day-panel active">
            ...
            <div class="hotel-showcase">
                <div class="hotel-showcase-info">
                    ...
                    <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(d.hotel + ' Japan')}" target="_blank" class="hotel-link">
                        查看地圖位置 →
                    </a>
                </div>
            ...
            <div class="hotel-simple">
                <span>🏨</span>
                <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(d.hotel + ' Japan')}" target="_blank">${d.hotel}</a>
    ...
    return meals.map((m, i) => `
        <div class="meal-item" data-index="${i}" onclick="focusOnMeal(${i}, ${m.lat}, ${m.lng})">
            <div class="meal-item-header">
                <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(m.name + ' Japan')}" target="_blank" onclick="event.stopPropagation()">
    ...
    return shops.map((s, i) => `
        <div class="shopping-item" data-index="${i}" onclick="focusOnShopping(${i}, ${s.lat}, ${s.lng})">
            <div class="shopping-header">
                <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(s.name + ' Japan')}" target="_blank" onclick="event.stopPropagation()">
...
function renderSpecialties(specs) {
    return specs.map((s, i) => `
        <div class="specialty-item" data-index="${i}" onclick="focusOnSpecialty(${i}, ${s.lat}, ${s.lng})">
            ...
            <div class="hours">🕒 ${s.hours}</div>
            <p>${s.desc}</p>
            <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(s.name + ' Japan')}" target="_blank" onclick="event.stopPropagation()">📍 Google Map</a>
        </div>
    `).join('');
}
...
function initLocalMap(d) {
    ...
    if (d.hotelLat && d.hotelLat !== 0) {
        ...
        L.marker([d.hotelLat, d.hotelLng], { icon: hotelIcon }).addTo(localMap)
            .bindPopup(`<div class="popup-day">🏨 今晚住宿</div><div class="popup-title">${d.hotel}</div><a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(d.hotel + ' Japan')}" target="_blank" style="font-size:11px;color:#E8A598">📍 Google Map</a>`);
    }
...
function updateMapMarkers(d) {
    ...
    mealMarkers.forEach(m => localMap.removeLayer(m));
    specialtyMarkers.forEach(m => localMap.removeLayer(m));
    shoppingMarkers.forEach(m => localMap.removeLayer(m));
    mealMarkers = [];
    specialtyMarkers = [];
    shoppingMarkers = [];

    if (currentSection === 'meals') {
        const meals = d.meals[currentMealType];
        const colors = { breakfast: '#FFB74D', lunch: '#FF7043', dinner: '#5C6BC0' };
        meals.forEach((m, i) => {
            if (m.lat && m.lat !== 0) {
                const icon = L.divIcon({
                    html: `<div class="meal-marker" style="background:${colors[currentMealType]}"><span>${i + 1}</span></div>`,
                    className: '', iconSize: [26, 26], iconAnchor: [13, 13]
                });
                const marker = L.marker([m.lat, m.lng], { icon }).addTo(localMap)
                    .bindPopup(`<div class="popup-day">${currentMealType === 'breakfast' ? '☀️ 早餐' : currentMealType === 'lunch' ? '🍛 午餐' : '🌙 晚餐'} ${i + 1}</div><div class="popup-title">${m.name}</div><div style="font-size:11px;color:#666">🕒 ${m.hours}</div>`);
                mealMarkers.push(marker);
            }
        });
        ...
        if (d.shopping) {
            d.shopping.forEach((s, i) => {
                if (s.lat && s.lat !== 0) {
                    const icon = L.divIcon({
                        html: `<div class="shopping-marker"><span>${i + 1}</span></div>`,
                        className: '', iconSize: [26, 26], iconAnchor: [13, 13]
                    });
                    const marker = L.marker([s.lat, s.lng], { icon }).addTo(localMap)
                        .bindPopup(`<div class="popup-day">🛒 購物 ${i + 1}</div><div class="popup-title">${s.name}</div><div style="font-size:11px;color:#666">🕒 ${s.hours}</div>`);
        ...
        d.specialties.forEach((s, i) => {
            if (s.lat && s.lat !== 0) {
                const icon = L.divIcon({
                    html: `<div class="specialty-marker"><span>${i + 1}</span></div>`,
                    className: '', iconSize: [24, 24], iconAnchor: [12, 12]
                });
                const marker = L.marker([s.lat, s.lng], { icon }).addTo(localMap)
                    .bindPopup(`<div class="popup-day">🎁 特產 ${i + 1}</div><div class="popup-title">${s.name}</div><div style="font-size:11px;color:#666">🕒 ${s.hours}</div>`);
...
function focusOnMeal(index, lat, lng) {
    ...
    if (mealMarkers[index]) mealMarkers[index].openPopup();
    document.getElementById('local-map')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
}
